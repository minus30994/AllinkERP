...
-- Description: 返回某个预算项目(辅助项目)的预算/下拨/报账数据
-- =============================================

Create Function F_Budget_Get_Value(
...
Begin
	...
	Begin
		...
		Begin
			...
			If @Assistant_Item_ID>=0 
				Set @Return=IsNull((Select Sum(D.FValue) From T_Budget_Allocate_Detail D With(NoLock) Inner Join T_Budget_Allocate_Header H With(NoLock) On D.FID=H.FID
	...
	Begin
		...
		Begin
			...
			If @Assistant_Item_ID>=0 
				Set @Return=IsNull((Select Sum(D.Famount) From T_Budget_Report_Detail D With(NoLock) Inner Join T_Budget_Report_Header H With(NoLock) On D.FID=H.FID
				Where
				H.FValue_Budget_Center_ID=@Budget_Center_ID
				And D.FBudget_Item_ID=@Budget_Item_ID
				And D.FAssistant_Item_ID=@Assistant_Item_ID
...
Begin
	...
	Begin
		If @Assistant_Item_ID>=0 
			...
			IsNull((Select
			Sum(d1.FAmount)
			From T_Budget_Report_Detail d1 With(NoLock)
